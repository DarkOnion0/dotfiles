...
def remoteIp [] {
    http get https://cloudflare.com/cdn-cgi/trace | lines | split column "=" field value | where field == ip or field == loc
}

# A command that generate a random anagram of the provided string
def anagramme [
    input: string # The string to randomize
    --repeat (-r): int = 1 # How many ouput should be printed
] {
    # Init vars (should be set there to share the edit acrros scope)
    export-env {
        $env.output = []
        $env.input = ""
    }

    $env.input = ($input | str trim | str downcase)

    export def str_random [input: string] {
        # Init vars (should be set there to share the edit acrros scope)
        export-env {
            $env.string = []
            $env.new_string = []
        ...
        $env.string = ($input | split row "" | skip | drop)
        $env.new_string = $env.string

        # Start randomiaztion
        for char in $env.string {
            loop {
                let index = (random int 0..(($env.string | length) - 1))

                if $index not-in $env.already_indexed {
                    $env.already_indexed = ($env.already_indexed | append $index)
                    $env.new_string = ($env.new_string | update $index $char)
                    break
                }
            }
        }
...
def diskusage [] {
    ...
    sys | get disks | select device type mount total removable

    # get the disk's index
    let disk_index = (input)

    # Display it's usage
    sys | get disks | select total free | get $disk_index | [(($in.total - $in.free) / $in.total) 100] | math product | math round -p 2
